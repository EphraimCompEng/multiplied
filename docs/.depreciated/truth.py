...
#
def operand_test_list(min_value: int =1, max_value: int =255, min_input_value: int =1,
    ...
    if int(max_value) < int(min_value):
        raise ValueError
    if int(max_value) < int(min_input_value):
        raise ValueError
    if min_value == 0:
    ...
    for y in input_range: # find MSB of Y -> Shift X by position -> IF > clamp: next
        ...
        for x in input_range:
            if (min_input_value <= x*y <= max_value) and (x << shift_distance) < clamp_value: # todo: dynamic parameter selection
...
def generate() -> None: # Build the json
    ...
    test_list = operand_test_list(255, 381, 3) # Generate operands
    # print(test_list)
    # print(len(test_list))
    data = {}
